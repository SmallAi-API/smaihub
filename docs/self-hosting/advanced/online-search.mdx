...
| `tavily`      | Web scraping and summarization API from [Tavily](https://www.tavily.com/).                                          | `TAVILY_API_KEY` `TAVILY_API_URL`                                           |
...
| `tavily`     | [Tavily](https://www.tavily.com/), offers fast web summaries and answers.               | `TAVILY_API_KEY` `TAVILY_API_URL`                                           |
...
Configures the API base URL shared by Tavily search and webpage extraction. The default is:

```env
TAVILY_API_URL=https://api.tavily.com
```

The application appends `/search` and `/extract` to this URL. When using a proxy, provide only the base URL without either endpoint path.
