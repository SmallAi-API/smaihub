...
| `tavily`      | 使用 [Tavily](https://www.tavily.com/) 的网页抓取与摘要 API。               | `TAVILY_API_KEY` `TAVILY_API_URL`                                           |
...
| `tavily`     | [Tavily](https://www.tavily.com/)，快速网页摘要与答案返回。                                        | `TAVILY_API_KEY` `TAVILY_API_URL`                                           |
...
配置 Tavily 搜索与网页抓取共用的 API 根地址。默认值如下：

```env
TAVILY_API_URL=https://api.tavily.com
```

系统会分别在该地址后追加 `/search` 和 `/extract`。使用代理服务时，请只填写根地址，不要包含这两个端点路径。
