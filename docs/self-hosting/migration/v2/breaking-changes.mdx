...
LobeHub 2.0 introduced the [pg\_search](https://github.com/paradedb/paradedb/tree/main/pg_search) extension for full-text search. When 2.0 shipped, Neon provided it to new PostgreSQL 17 projects. Neon has since stopped offering `pg_search` to new projects. Retirement timelines for existing projects follow the notice Neon sends for each project. LobeHub users who receive such a notice should complete [Migrate from pg\_search on Neon](/docs/self-hosting/advanced/neon-pg-search-migration) before the deadline in that notice. See [Full-Text Search](/docs/self-hosting/advanced/full-text-search) before choosing the search backend for a current deployment.
