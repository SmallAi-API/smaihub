...
<Callout type="info">
  如果你在 CDN（如 Cloudflare）或反向代理后部署 LobeHub，你可以配置内部服务器到服务器通信以绕过 CDN
  / 代理层，以获得更好的性能。
</Callout>
...
<Callout type="tip">
  对于使用 `network_mode: 'service:network-service'` 的 Docker Compose 部署，请使用
  `http://localhost:3210` 作为 `INTERNAL_APP_URL`。
</Callout>
